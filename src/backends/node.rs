...
/// translation — no runtime value type is needed. the tools runtime lives in
/// runtime/tools/tools.js (written next to the output by the CLI) and is
...
pub fn generate(ast: &Spanned<Node>) -> String {
    ...
    out.push_str("const _tools = require('./tools.js');\n");
    out.push_str("const { fmt, pucho, truthy, intezar, taqseem, baaqi,\n");
    ...
    out.push_str("        mafta, qeemtain, key_hai, hata } = _tools;\n");
...
fn gen_call(name: &str, args: &[Spanned<Node>]) -> String {
    ...
    // Higher-order tools: the kid's `x`-expression becomes an arrow fn.
