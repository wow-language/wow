...
pub fn generate(ast: &Spanned<Node>) -> String {
    ...
    let uses_wifi = stmts.iter().any(uses_wifi_in);

    let mut out = String::new();
    out.push_str("// Generated by wow — Code likho. Wow bolo.\n");
    // AUZAAR_ESP32 activates the WiFi/WebServer section of the header without
    // relying on the toolchain defining ESP32 before the header is parsed.
    if uses_wifi {
        out.push_str("#define AUZAAR_ESP32\n");
    }
...
fn uses_wifi_in(node: &Spanned<Node>) -> bool {
    match &node.node {
        Node::Call { name, .. } => matches!(
            name.as_str(),
            "wifi_jodo" | "wifi_ip" | "server_rasta" | "server_shuru" | "server_parho" | "jawab_bhejo"
        ),
        Node::ArduinoShuru(body) | Node::ArduinoChalao(body) | Node::Banao { body, .. } => {
            body.iter().any(uses_wifi_in)
        }
        Node::Agar { then_body, else_ifs, else_body, .. } => {
            then_body.iter().any(uses_wifi_in)
                || else_ifs.iter().any(|(_, b)| b.iter().any(uses_wifi_in))
                || else_body.as_ref().map_or(false, |b| b.iter().any(uses_wifi_in))
        }
        Node::HarRange { body, .. } | Node::Baar { body, .. } | Node::Jabtak { body, .. } => {
            body.iter().any(uses_wifi_in)
        }
